...
import { ShieldCheck, Clock, Wallet, FileWarning, FileCheck2, Paperclip } from 'lucide-react';
            ...
            <Card key={b.id} className={`rounded-[18px] border shadow-soft ${isLowest ? 'border-[#00B59E] bg-[#D0F2EE]/10 ring-1 ring-[#00B59E]/15 dark:bg-[#00B59E]/[0.04]' : 'border-border'}`}>
              <CardContent className="p-4 sm:p-5">
                ...
                <div className="mt-3 space-y-1.5">
                  {b.warranty && <div className="flex items-start gap-2 text-[13px] leading-relaxed"><FileCheck2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-navy" aria-hidden="true" /><span className="min-w-0 break-words"><span className="font-semibold">{t('warranty')}:</span> {b.warranty}</span></div>}
                  {b.exclusions && <div className="flex items-start gap-2 text-[13px] leading-relaxed"><FileWarning className="mt-0.5 h-3.5 w-3.5 shrink-0 text-navy" aria-hidden="true" /><span className="min-w-0 break-words"><span className="font-semibold">{t('exclusions')}:</span> {b.exclusions}</span></div>}
                  ...
                  <div className="mt-3 border-t border-border pt-3">
                    ...
                    <div className="space-y-1.5">
                      {b.attachments.map((f, i) => (
                        <ResultFileLink key={i} file={f} fallbackLabel={t('files')} newTab />
                      ))}
                    </div>
                  </div>
                )}
